...
categorizer::categorizer(int _clusters)
{
	...
	clusters=_clusters;
	//初始化指针
	featureDecter = ORB::create();
	bowtrainer = new BOWKMeansTrainer(clusters);
	descriptorMacher = BFMatcher::create(NORM_HAMMING);
...
void categorizer::compute_bow_image()
{
	...
	if(va_fs.isOpened())
	{
		...
		if (temp_vacab.type() != CV_8U)
  	{
    	temp_vacab.convertTo(temp_vacab, CV_8U);
  	}
